...
This release is the 2.x general availability launch of the Android SDK, which includes a number of significant new features that are now stable and fully supported.

### Upgrading to 2.1.0
This release makes [Feature Management](https://developers.optimizely.com/x/solutions/sdks/reference/?language=android#feature-introduction) generally available, which replaces the SDK's variable APIs (`getVariableBoolean`, etc.) with the feature variable APIs (`getFeatureVariableBoolean`, etc.).  The primary difference between the new Feature Variable APIs and the older, Variable APIs is that they allow you to link your variables to a Feature (a new entity defined in the Optimizely UI) and to a feature flag in your application.  This in turn allows you to run Feature Tests and Rollouts on both your Features and Feature Variables.  For complete details of the Feature Management APIs, see the "New Features" section below.
* *Note*: As of version 2.1.0, the Variable APIs are deprecated. We will continue to support the Variable APIs until the 3.x release, but we encourage you to upgrade as soon as possible.
...
### New Features
* Introduces the `isFeatureEnabled` API, a featue flag used to determine whether to show a feature to a user. The `isFeatureEnabled` can be used in place of the `activate` API to activate experiments.  Calling this API causes the SDK to evaluate all Feature Tests and Rollouts associated with the provided feature key.
```
