...
class EventDispatcher(private val context: Context, private val optlyStorage: OptlyStorage, private val eventDAO: EventDAO, private val eventClient: EventClient, private val serviceScheduler: ServiceScheduler, private val logger: Logger) {
    fun dispatch(intent: Intent) {
        ...
        if (intent.hasExtra(EventIntentService.Companion.EXTRA_URL)) {
            try {
                val urlExtra = intent.getStringExtra(EventIntentService.Companion.EXTRA_URL)
                val requestBody = intent.getStringExtra(EventIntentService.Companion.EXTRA_REQUEST_BODY)
    ...
    private fun getInterval(intent: Intent): Long {
        ...
        return intent.getLongExtra(EventIntentService.Companion.EXTRA_INTERVAL, -1)
    }

    private fun saveInterval(interval: Long) {
        optlyStorage.saveLong(EventIntentService.Companion.EXTRA_INTERVAL, interval)
    }
