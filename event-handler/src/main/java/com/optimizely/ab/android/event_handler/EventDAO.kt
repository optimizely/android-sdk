 ...
 */ private constructor(private val dbHelper: EventSQLiteOpenHelper, val logger: Logger) {
    ...
    fun storeEvent(event: Event): Boolean {
        ...
        values.put(Column.URL, event.uRL.toString())
        values.put(Column.REQUEST_BODY, event.requestBody)
...
// Define a projection that specifies which columns from the database
     ...
     */
    val events: MutableList<Pair<Long, Event>>
        get() {
                ...
                if (cursor != null && cursor.moveToFirst()) {
                    do {
                        ...
                        val url = cursor.getString(
                                cursor.getColumnIndexOrThrow(Column.URL)
                        )
                        val requestBody = cursor.getString(
                                cursor.getColumnIndexOrThrow(Column.REQUEST_BODY)
                        )
                        try {
    ...
    companion object {
        ...
        fun getInstance(context: Context, projectId: String, logger: Logger): EventDAO {
            val sqLiteOpenHelper = EventSQLiteOpenHelper(context, projectId, null, EventSQLiteOpenHelper.Companion.VERSION, LoggerFactory.getLogger(EventSQLiteOpenHelper::class.java))
